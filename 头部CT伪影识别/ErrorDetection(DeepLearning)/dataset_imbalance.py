...
def data_load(path, test_path, size, is_train, is_sampling='no_sampler'):
    dicomlist = []    # Image paths
    labels = []       # Image labels
    train_img = []    # Training set images
    train_label = []  # Training set labels
    val_img = []      # Validation set images
    val_label = []    # Validation set labels

    # 1. Read data: images and labels
    f = open(path, "r+") if test_path == '' else open(test_path, "r+")
    for line in f.readlines():
        img_path = line.strip().split(',')[0]  # Image path
        dicomlist.append(img_path)
        label = line.strip().split(',')[1]  # Image label
        label = '0' if label == 'good' else '1'
        labels.append(label)
    labels = np.array(labels)  # Image labels n*1
    # Read image matrices
    ...
    if is_train or test_path == '':  # Training or testing mode without separate csv
        print('----Training Mode----') if is_train else print('----Testing mode----')

        # Split class `1` samples into train, validation, and test sets
        class_1_train_img, class_1_val_test_img, class_1_train_label, class_1_val_test_label = train_test_split(
            class_1_images, class_1_labels, test_size=0.5, random_state=random_seed)

        class_1_val_img, class_1_test_img, class_1_val_label, class_1_test_label = train_test_split(
            class_1_val_test_img, class_1_val_test_label, test_size=0.5, random_state=random_seed)

        # Split class `0` samples into train, validation, and test sets
        class_0_train_val_img, class_0_test_img, class_0_train_val_label, class_0_test_label = train_test_split(
            class_0_images, class_0_labels, test_size=ratio, stratify=class_0_labels, random_state=random_seed)

        class_0_train_img, class_0_val_img, class_0_train_label, class_0_val_label = train_test_split(
            class_0_train_val_img, class_0_train_val_label, test_size=ratio, stratify=class_0_train_val_label, random_state=random_seed)

        # Combine the two classes into final sets
        train_img = np.concatenate((class_0_train_img, class_1_train_img), axis=0)
        train_label = np.concatenate((class_0_train_label, class_1_train_label), axis=0)
        val_img = np.concatenate((class_0_val_img, class_1_val_img), axis=0)
        val_label = np.concatenate((class_0_val_label, class_1_val_label), axis=0)
        test_img = np.concatenate((class_0_test_img, class_1_test_img), axis=0)
        test_label = np.concatenate((class_0_test_label, class_1_test_label), axis=0)

        # Shuffle each set to mix class 0 and class 1 samples
...
# 上采样
def over_sampling(train_img, train_label):
    nsamples, nx, ny, nz = train_img.shape  # 获取原始图像的形状信息
    train_img_flatten = train_img.reshape(nsamples, nx * ny * nz)  # 将图像展平成2D形式
    smote = SMOTE(random_state=random_seed, k_neighbors=1)  # 创建SMOTE实例
    X_resampled, y_resampled = smote.fit_resample(train_img_flatten, train_label)  # 使用SMOTE进行上采样
    X_resampled = X_resampled.reshape(X_resampled.shape[0], nx, ny, nz)  # 重新调整为原始形状
    return X_resampled, y_resampled
