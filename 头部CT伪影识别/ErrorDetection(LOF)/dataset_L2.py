...
# 图像预处理（伪影增强）
def data_preprocess_enhanced(img, size, is_windows_pro=False, is_equalize_hist=False):
...
#       读取数据并划分数据集
# -------------------------------#
def data_load(path, test_path, size, is_train, is_sampling='no_sampler', is_windows_pro=False, is_equalize_hist=False):
    dicomlist = []    # 图像地址
    labels = []       # 图像标签
    train_img = []    # 训练集图像
    train_label = []  # 训练集标签
    ...
    # 读取图像矩阵
    images = array([data_preprocess_enhanced(pydicom.read_file(dcm).pixel_array, size, is_windows_pro, is_equalize_hist) for dcm in dicomlist])
    f.close()
...
#          加载数据集
# -------------------------------#
def get_dataset(path, test_path, size, batch_size, is_train, is_sampling=False, is_windows_pro=False, is_equalize_hist=False):
    print('启用图像阈值范围缩减' if is_windows_pro else '停用图像阈值范围缩减')
    print('启用直方图均衡' if is_equalize_hist else '停用直方图均衡')
    train_img, train_label, val_img, val_label, test_img, test_label = data_load(path, test_path, size, is_train,
                                                                                 is_sampling, is_windows_pro, is_equalize_hist)
    train_loader = []
    val_loader = []
    ...
    if is_train:
        ...
        train_data_transform = transforms.Compose([
            ...
            transforms.RandomRotation((0, 180), resample=False, expand=False),
            transforms.ToTensor()])
